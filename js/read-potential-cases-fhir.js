...
(async () => {
  ...
  await fs.writeFile("immunodeficiencies-potential-cases.csv", "patient-id,dob,codes,last-encounter\n");
  for(let patient in patients) {
    try {
      ...
      await fs.appendFile("immunodeficiencies-potential-cases.csv", row);
    } catch(error) {
      console.log(error);
    }
  }
